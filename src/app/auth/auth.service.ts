...
export class AuthService{
  ...
  login(email: string, password: string){
    return this.http.post<AuthResponseData>(
      'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=2' + environment.firebaseAPIKey,
      {
        email: email,
        password: password,
        returnSecureToken: true
  ...
  autoLogin(){
    ...
    const loadedUser = new User(userData.email, userData.id, userData._token, new Date(userData._tokenExprirationDate));
